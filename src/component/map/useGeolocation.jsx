...
import {fetchLocation, getSurveyInfoForBlog} from "../../api/survey";
...
const useGeolocation = (map, onValidMarkerClick) => {
    ...
    const validateLocation = (databaseLocation, myLocation) => {
        ...
        return distance <= 1000; // 1km 내에 있는 경우 true, 그렇지 않은 경우 false
    };
    ...
    useEffect(() => {
        ...
        const fetchSurveyInfo = async () => {
            ...
            if (surveyIds.length > 0) {
                try {
                    const response = await getSurveyInfoForBlog(surveyIds);
